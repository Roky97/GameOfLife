...
//GESTIONE GRAFICA
const int FPS=30; //frame al secondo
...
int dimQuad=5; // dimenzione del lato una singola casella in pixel. (5 o 10 o 20)
...
void evolveMatrice(bool a[h1][w1], const int & w, const int & h, const int & nThreads, int id) //funzione principale
{
    ...
    {
        int pos = nRows*i;
        MPI_Isend(&(a[pos][0]), 1, totRighe, i, 0, MPI_COMM_WORLD, &req);
    }
    ...
    for(int i=1; i<nThreads; i++)//per ogni thread manda i bordi delle matrice "ghost points" necessari per determinare le adiacence esatte per ogni punto della matrice
    {
        ...
        {
            int posIn=(nRows*i) -1 ;
            MPI_Isend(&(a[posIn][0]), 1, riga, i, 0, MPI_COMM_WORLD, &req); // ghost point inizio

            int posFin=nRows*(i+1);
            MPI_Isend(&(a[posFin][0]), 1, riga, i, 0, MPI_COMM_WORLD, &req); // ghost point fine
        }
    ...
    for( int i=1; i<nThreads; i++) //per ogni thread ricevo la porzione di matrice evoluta
    {
        ...
        MPI_Irecv(&(a[pos][0]), nElements, MPI_CXX_BOOL, i, 0, MPI_COMM_WORLD, &req);
        MPI_Wait(&req, &stat);
...
int main(int argc, char **argv) {
    ...
    {
        init(); // inizializzo le fuznionalita' di allegro
        ...
        int r=255, g=0, b=0; //colori delle celle sul display
        ...
        while(!doexit && contCicli<cicli)
        {
            ...
            if(ev.type == ALLEGRO_EVENT_TIMER)
            {
                ...
                for(int i=1; i<nThreads; i++)
                    MPI_Isend(&doexit, 1, MPI_CXX_BOOL, i, 1, MPI_COMM_WORLD, &req); //spedisco la booleana del ciclo di gioco agli slaves
    ...
    if(id!=0) //CODICE ESEGUITO DAI POCESSI SLAVES
    {
        while(!doexit)
        {
            ...
            MPI_Irecv(&subMatrix[0][0], nElements, MPI_CXX_BOOL, 0, 0, MPI_COMM_WORLD, &req); //riceve la porzione di matrice dal master

            if(id<nThreads-1)
            {
                MPI_Irecv(ghostIn, w, MPI_CXX_BOOL, 0, 0, MPI_COMM_WORLD, &req); // ghost point inizio

                MPI_Irecv(ghostFin, w, MPI_CXX_BOOL, 0, 0, MPI_COMM_WORLD, &req); // ghost point fine
            }
            ...
            MPI_Isend(&ad[0][0], 1, totRighe, 0, 0, MPI_COMM_WORLD, &req); //spedisco la porzione di matrice aggiornata al master

            MPI_Irecv(&doexit, 1, MPI_CXX_BOOL, 0, 1, MPI_COMM_WORLD, &req); // ricevo la booleana del ciclo di gioco dal master
            MPI_Wait(&req, &stat);
        }
    }
